...
class Node:
    def __init__(self, args:'dict', state:'Kita', move_counter:'int', depth:'int'=0, parent:'Node | None'=None, action:'int|None'=None, prior:'float|None'=None, policy:'np.ndarray|None'=None):
        self.args = args
        self.state = state
        self.parent = parent
        self.prior = prior
    ...
    @torch.no_grad()
    def value_policy_batch(self, states: 'list[Kita]', move_counter: 'int ', model: 'ResNet') -> tuple[np.ndarray, np.ndarray]:
        
        model.eval()
        inputs = torch.stack([f.prepare_input(state, move_counter) for state in states]).to(self.args['device'])
...
class MCTS:
    ...
    @torch.no_grad()
    def value_policy(self, state: 'Kita', move_counter: 'int') -> tuple[float, np.ndarray]:
       
        self.model.eval()
        value , policy = self.model(f.prepare_input(state, move_counter).unsqueeze(0).to(self.args['device']))
    ...
    def _simulate(self, root: 'Node') -> None:
        ...
        if current_node.is_terminal():
            value = -f.game_result(current_node.state, current_node.move_counter, 1000)[0]*1000
    ...
    def search(self, state: 'Kita', move_counter: 'int') -> np.ndarray:
        max_depth.clear()
